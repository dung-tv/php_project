                                ...
                                <td class="cart-col-qty">
                                    <div class="cart-qty">
                                        ...
                                        <input style="width: 70px;" type="number" data-id="{{ $item->id }}"
                                            ...
                                            class="item-qty" name="qty-{{$item->id}}" value="{{$item->qty}}" min="0">
    ...
    function updateCart(obj){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                {
                    if (new_qty == 0){
                        var del = obj.parent().parent().next().next().find('a:first-child');
                        window.location = del.prop('href');
                    }
                    obj.parent().parent().next().text(data.newPrice);
                    data.data.forEach(function(item){
                        if(item.code === 'subtotal') {
                            $('#subtotal').text(item.text)
                        }
                        if(item.code === 'tax') {
                            $('#tax').text(item.text)
                        }
                        if(item.code === 'total') {
                            $('#total').text(item.text)
                        }
                    })
