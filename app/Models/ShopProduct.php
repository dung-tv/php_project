...
class ShopProduct extends Model
{
    ...
    protected  $sc_order_detail = 0; // 1: only product most order,
    protected  $sc_check_has_promotion = ''; // array check has promotion
    ...
    {
        return $this->hasMany(ShopOrderDetail::class, 'product_id', 'id');
    }
...
//Scort
    ...
    */
    ...
    /**
     * Get product most order
     */
    public function getProductOrder($limit, $has, $array_product_promotion) {
        $this->setLimit($limit);
        $this->sc_order_detail = $has;
        $array_product_promotion = $array_product_promotion->toArray();
        $this->sc_check_has_promotion = implode(', ', !empty($array_product_promotion) ? $array_product_promotion : [0]);
        return $this;
    }
    ...
    public function buildQuery() {
        ...
        //Most order - Order detail
        if ($this->sc_order_detail == 1 && $this->sc_check_has_promotion !== '') {
            $tableOrderDetail = (new ShopOrderDetail)->getTable();
            $query = $query->selectRaw($this->getTable() . '.*, ' . $tableDescription . '.*, count(' . $tableOrderDetail . '.product_id)')->join(
                $tableOrderDetail,
                $this->getTable() . '.id', '=', $tableOrderDetail . '.product_id')
                ->whereRaw($this->getTable() . '.id not in(' . $this->sc_check_has_promotion . ')')
                ->groupByRaw($tableOrderDetail . '.product_id')
                ->orderByRaw('count(' . $tableOrderDetail . '.product_id) desc');
            $query = $query->with('mostOrder');
        }
