...
class ShopCart extends GeneralController
{
    ...
    public function addToCartAjax(Request $request)
    {
        ...
        switch ($instance) {
            case 'default':
                ...
                //Check product out_of_stock
                if ($product->stock <= 0 && !sc_config('product_buy_out_of_stock')) {
                    return response()->json(
                        [
                            'error' => 1,
                            'msg' => trans('cart.out_of_stock', ['item' => $product->sku]),
                        ]
                    );
                }
    ...
    public function updateToCart(Request $request)
    {
        ...
        elseif ($product->stock < $new_qty && !sc_config('product_buy_out_of_stock')) {
            return response()->json(
                [
                    'error' => 1,
            ...
            $inputRowId = ( $new_qty > 0 ) ? Cart::instance('default')->get($rowId)->price : 0;
            $objects = ShopOrderTotal::getObjectOrderTotal();
            $data = ShopOrderTotal::processDataTotal($objects);
            return response()->json(
                [
                    'error' => 0,
                    'newPrice' => sc_currency_render($inputRowId * $new_qty),
                    'data' => $data,
                    'raw' => $inputRowId,
                ]
            );
